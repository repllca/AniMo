...
def evaluation_vqvae(out_dir, val_loader, net, writer, ep, best_fid, best_div, best_top1,
    ...
    for batch in val_loader:
        ...
        pred_pose_eval, loss_commit, perplexity = net(species,gender,motion)
...
def evaluation_vqvae_plus_mpjpe(val_loader, net, repeat_id, eval_wrapper, num_joint):
    ...
    for batch in val_loader:
        # print(len(batch))
        word_embeddings, pos_one_hots,species,gender, caption, sent_len, motion, m_length, token = batch
...
def evaluation_base_transformer(out_dir, val_loader, trans, vq_model, writer, ep, best_fid, best_div,
    ...
    for batch in val_loader:
        ...
        bs, seq = pose.shape[:2]
        mids = trans.generate(clip_text,species,gender, m_length//4, time_steps, cond_scale, temperature=1)
        mids.unsqueeze_(-1)
...
def evaluation_base_transformer_test_plus_res(val_loader, vq_model, res_model, trans, repeat_id, eval_wrapper,
    ...
    for i, batch in enumerate(val_loader):
        ...
        # (b, seqlen, c)
            motion_multimodality_batch = []
            for _ in range(30):
                mids = trans.generate(clip_text,species,gender, m_length // 4, time_steps, cond_scale,
                                      temperature=temperature, topk_filter_thres=topkr,
                                      gsample=gsample, force_mask=force_mask)
