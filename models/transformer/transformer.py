...
class BaseTransformer(nn.Module):
    ...
    def trans_forward(self, motion_ids, cond,species,gender, padding_mask, force_mask=False):
        '''
        ...
        species = self.cond_emb(species).unsqueeze(0)
        gender = self.cond_emb(gender).unsqueeze(0) 
        
        x = self.position_enc(x)   
        xseq = torch.cat([cond,species,gender, x], dim=0)
        padding_mask = torch.cat([torch.zeros_like(padding_mask[:, 0:3]), padding_mask], dim=1) 
        output = self.seqTransEncoder(xseq, src_key_padding_mask=padding_mask)[3:] 
        logits = self.output_process(output) 
        return logits

    def forward(self, ids, y,species,gender, m_lens):
        '''
        :param ids: (b, n)
        ...
        if self.cond_mode == 'text':
            with torch.no_grad():
                ...
                species = self.encode_text(species)  #
                gender = self.encode_text(gender)
        elif self.cond_mode == 'action':
        ...
        logits = self.trans_forward(x_ids, cond_vector,species,gender, ~non_pad_mask, force_mask)
    ...
    def forward_with_cond_scale(self,
        ...
        logits = self.trans_forward(motion_ids, cond_vector, species,gender,padding_mask)
        if cond_scale == 1:
            return logits

        aux_logits = self.trans_forward(motion_ids, cond_vector, species,gender,padding_mask, force_mask=True)  
    ...
    def generate(self,
        ...
        for timestep, steps_until_x0 in zip(torch.linspace(0, 1, timesteps, device=device), reversed(range(timesteps))):
            ...
            '''
            # (b, num_token, seqlen)
            logits = self.forward_with_cond_scale(ids, cond_vector=cond_vector,species=species,gender=gender,
...
class ResidualTransformer(nn.Module):
    ...
    def trans_forward(self, motion_codes, qids, cond,species,gender, padding_mask, force_mask=False):
        '''
        ...
        gender = self.cond_emb(gender).unsqueeze(0) 
        x = self.position_enc(x)
        xseq = torch.cat([cond,species,gender, q_emb, x], dim=0)  
        padding_mask = torch.cat([torch.zeros_like(padding_mask[:, 0:4]), padding_mask], dim=1)  
        output = self.seqTransEncoder(xseq, src_key_padding_mask=padding_mask)[4:]  
        logits = self.output_process(output)    
        return logits
    ...
    def forward_with_cond_scale(self,
        ...
        logits = self.trans_forward(motion_codes, qids, cond_vector,species,gender, padding_mask)
        logits = self.output_project(logits, qids-1)
        if cond_scale == 1:
            return logits

        aux_logits = self.trans_forward(motion_codes, qids, cond_vector,species,gender, padding_mask, force_mask=True)
        aux_logits = self.output_project(aux_logits, qids-1)

        scaled_logits = aux_logits + (logits - aux_logits) * cond_scale
        return scaled_logits

    def forward(self, all_indices, y,species,gender, m_lens):
        '''
        ...
        logits = self.trans_forward(history_sum, active_q_layers, cond_vector,species,gender, ~non_pad_mask, force_mask)
    ...
    def generate(self,
        ...
        for i in range(1, num_quant_layers):
            ...
            logits = self.forward_with_cond_scale(history_sum, i, cond_vector, species,gender,padding_mask, cond_scale=cond_scale)
            logits = logits.permute(0, 2, 1)  # (b, seqlen, ntoken)
            filtered_logits = top_k(logits, topk_filter_thres, dim=-1)
