...
class BaseTransformerTrainer:
    ...
    def forward(self, batch_data):
        ...
        species,gender,conds, motion, m_lens = batch_data  
        motion = motion.detach().float().to(self.device)
        m_lens = m_lens.detach().long().to(self.device)

        # (b, n, q)
        code_idx, _ = self.vq_model.encode(species,gender,motion)  
        m_lens = m_lens // 4

        conds = conds.to(self.device).float() if torch.is_tensor(conds) else conds
        species = species.to(self.device).float() if torch.is_tensor(species) else species
        gender = gender.to(self.device).float() if torch.is_tensor(gender) else gender
        _loss, _pred_ids, _acc = self.t2m_transformer(code_idx[..., 0], conds, species,gender,m_lens)
        return _loss, _acc
...
class ResidualTransformerTrainer:
    ...
    def forward(self, batch_data):
        ...
        # (b, n, q), (q, b, n ,d)
        code_idx, all_codes = self.vq_model.encode(species,gender,motion)
        m_lens = m_lens // 4
        ...
        ce_loss, pred_ids, acc = self.res_transformer(code_idx, conds,species,gender, m_lens)
